...
class DDPG_PER(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001, total_timesteps=0):
        ...
        for it in range(iterations):
            ...
            next_state = torch.FloatTensor(obses_tp1).to(device)
            ...
            ### Get new priorities
            new_priorities = np.abs(td_errors.cpu().detach().numpy())[0][0] + prioritized_replay_eps
            print(new_priorities)
            replay_buffer.update_priorities(batch_idxes, new_priorities)

            ### Compute weighted critic loss
            weights_squared = torch.FloatTensor(np.sqrt(weights)).to(device)
            critic_loss = F.mse_loss(weights_squared*current_Q, weights_squared*target_Q)
